...
( function( $ ) {
	...
	function _sale_nofication_start(){
		if($(".sale-nofication").length){
			...
			$.ajax({
				url: keny_ajax.ajaxurl,
				type: 'POST',
				dataType: 'json',
				data: {
					action : "keny_time_nofication_ajax",
					id_product : id_product,
					security : keny_ajax.ajax_nonce
				},
	...
	function _headercategories(){
		...
		$('.keny-close',$menu_categories).on( "click", function() {
			$menu_categories.removeClass('active');
			return false;
		});		
		//Navigation right			
	...
	function _canvasrightNavigation(){
		...
		$('.keny-close',$wpbingo_menu_right).on( "click", function() {
			$wpbingo_menu_right.removeClass('active');
			return false;
		});		
		//Navigation right			
		...
		$(".input-search",$element).on("keydown", function() {
			...
			if(character.length >= 2){
				...
				$.ajax({
					...
					dataType: 'json',
					data: {
						action : "keny_search_products_ajax",
						character : character,
						limit : limit,
						category : category,
						security : keny_ajax.ajax_nonce
	...
	function _load_360_view_product(){
		...
		$(".keny-360-button").on( "click", function() {
	...
	function _after_add_to_cart(){ 
		...
		$( _body ).on( 'added_to_cart', function(){
			if(wd_width >= 1199){
				var $element = $(".keny-topcart-desktop");
			}else{
				var $element = $(".keny-topcart-mobile");
			}
	...
	function _ajax_cart_header(){
		var wd_width = _window.width();
		if(wd_width >= 1199){
			var topcart_form = $(".keny-topcart-desktop");
		}else{
			var topcart_form = $(".keny-topcart-mobile");
		}
	...
	function _click_add_to_cart(){
		var topcart_form = $(".keny-topcart-mobile,.keny-topcart-desktop");
		topcart_form.on('click', '.cart-icon', function(e){
			...
			if( !_body.hasClass("not-scroll") && $(".keny-topcart").hasClass("popup") ){
				_body.addClass("not-scroll");
			}			
		});
			...
			$.ajax({
				...
				success: function(data){
					...
					if (data.loggedin == true){
						...
						document.location.href = keny_ajax.redirecturl;
					}else{
	...
	class Elementor_Js_keny {
		static getInstance() {
			if (!Elementor_Js_keny.instance) {
				Elementor_Js_keny.instance = new Elementor_Js_keny();
			}
			return Elementor_Js_keny.instance;
		}
		constructor() {

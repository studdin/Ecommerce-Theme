...
function keny_custom_css() {
	...
	if (!is_admin()) {
		...
		wp_add_inline_style( 'keny-style-template', implode($custom_css) );
	}
}
add_action('wp_enqueue_scripts', 'keny_custom_css' );
function keny_custom_js() {
	if (!is_admin()) {
		wp_enqueue_script( 'keny-script', get_template_directory_uri() . '/js/functions.js', array( 'jquery'), null, true );
		wp_localize_script( 'keny-script', 'keny_ajax',
			array( 
				'ajaxurl' => admin_url( 'admin-ajax.php' ),
				'redirecturl' => home_url(),
				'ajax_nonce' => wp_create_nonce('keny_ajax_nonce'),
			)
		);
		...
		wp_add_inline_script( 'keny-script', $custom_js);
	}
}
add_action('wp_enqueue_scripts', 'keny_custom_js' );